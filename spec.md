...
| **cua-driver 데몬 통합** | agent `cua.rs` 가 데몬 수명주기를 **3중**으로 보장: (1) startup `ensure_daemon()`(status→없으면 `serve` detached 기동+준비 폴링), (2) `browser::ensure()` 가 window 조회 전 보장, (3) `run_gui` 가 "daemon is not running" 감지 시 되살려 1회 재시도(자가교정). **로그온 스케줄 작업(`autostart enable`) 등록은 제거**(커밋 아래) — agent 가 HKCU Run 으로 이미 로그온마다 뜨고 그때 (1)이 데몬을 띄우므로 잉여인데, 등록 작업이 `RunLevel=Highest` 라 특권 경계를 건드리고 startup 을 늦춘다 | E2E (데몬 kill→startup 복구 / mid-session 자동복구+재시도). **실측(학생 노트북 로그)**: `tailscale already connected`(15:28:14.770) → `autostart enable ok=true`(15:28:38.708) = **이 호출 한 줄이 23.9초**, Hello 가 그만큼 지연 |
...
| **중복 실행 내성 (agent 단일 인스턴스 + hub 연결단위 정리)** | 실사고: 학생 노트북에 agent 가 2개 떠 있었다(`pid 7728` HKCU Run 자동시작 + `pid 11812` explorer.exe 에서 수동 실행). GameStream 6포트는 먼저 뜬 쪽이 잡아 두 번째는 스트리밍 불가인데, hub WS 는 **둘 다** 붙어 같은 `agent_id` 로 등록을 뺏고 뺏긴다. ① **agent**: `main.rs::claim_single_instance()` 가 named mutex `Local\kmc-agent-singleton` 을 잡고, `ERROR_ALREADY_EXISTS` 면 경고 후 즉시 종료(뮤텍스 생성 자체가 실패하는 환경이면 가드를 포기하고 진행 — 가드 때문에 agent 가 안 뜨는 게 더 나쁘다). ② **hub**: `AgentConn` 에 단조증가 `conn_id` 를 넣고 `cleanup(state, agent_id, conn_id)` 이 **현행 등록이 내 conn_id 일 때만** 정리한다. 이전엔 늦게 끊긴 옛 연결이 살아있는 새 연결의 주소·제어세션까지 지워 **agent 가 오프라인으로 오표시**됐다 | **agent 가드 실측**(PowerShell 로 같은 이름 뮤텍스 선점 후 실 바이너리 기동): 선점 시 `kmc-agent 가 이미 실행 중이다` 경고 후 즉시 종료, 미선점 시 가드를 통과해 provision 까지 진행. **hub 실측**(임시 hub + 같은 agent_id WS 2개): ws1 접속 → ws2 로 교체(`old_conn=1 new_conn=2`) → **ws1 종료 시 `stale agent conn closed` 만, `agent offline` 없음** → ws2 종료에서야 `agent offline` |
...
- **GUI 자동화는 cua-driver(trycua, MIT) 차용**: UIA 백그라운드 조작을 재구현하지 않고 CLI 셸아웃으로 브리지. 사양의 "PostMessage 백그라운드 우선" 원칙과 부합. agent가 `%LOCALAPPDATA%\Programs\Cua\...\cua-driver.exe` 자동 탐색.
  - **데몬 상시 보장(agent 통합)**: GUI/브라우저 자동화가 전부 cua-driver 데몬(`\\.\pipe\cua-driver`)을 거치므로 데몬이 죽으면 전 조작이 실패한다. agent `cua.rs`가 세 겹으로 보장 — (1) startup `ensure_daemon()`: `status`로 확인 후 없으면 `serve`를 `DETACHED_PROCESS`로 기동하고 준비 폴링, (2) `browser::ensure()`가 window 조회(cua list_windows) 전에 데몬을 보장, (3) 런타임 자가교정: `run_gui`가 호출 결과에서 "daemon is not running"을 감지하면 `ensure_daemon()` 후 1회 재시도해 호출자에게 실패를 노출하지 않음.
    - **로그온 스케줄 작업(`autostart enable`)은 제거했다**: agent 자체가 HKCU Run 으로 로그온마다 뜨고 그때 (1)이 데몬을 띄우므로 순수 잉여인데, 등록되는 작업은 `RunLevel=Highest`(승격 실행)라 특권 경계를 건드리고, startup 마다 호출하면 그만큼 기동이 늦다 — 실측 학생 노트북 로그에서 **이 한 줄이 23.9초**(`15:28:14.770` → `15:28:38.708`). "특권 작업은 elevated 인스톨러만" 규칙(`tailscale.rs` 의 `up` 제거와 동일)을 여기에도 적용.
...
- **임베디드 Tailscale 재검토: `GeiserX/tailscale-rs` — 조건부 보류(2026-07-26 평가)**. 순수 Rust tsnet 포트(`geiserx_tailscale` 0.6, BSD-3, upstream `tailscale/tailscale-rs` 의 포크). 매력은 분명하다: Go/cgo 없음, **Windows x86_64 지원**, tokio 풍 API 로 `udp_bind`/`listen_packet`(→ `netstack::UdpSocket`) 가 있어 **데이터그램 경계가 보존**되고(=RTP/FEC 설계를 그대로 이식 가능), 전부 netstack 으로 가면 **OS 포트를 하나도 안 열어 WinTun 드라이버도 방화벽 규칙도 불필요 → 설치 UAC 가 0 이 된다**. 그럼에도 지금은 채택 불가:
  1. **symmetric-NAT 홀펀칭 미구현** (README Caveats: *"Symmetric-NAT birthday-paradox hole-punching is not yet implemented, so behind some NATs a flow stays relayed through DERP, which caps its throughput"*). 네이티브 Tailscale 에는 있다. 우리 제품의 본질이 2880×1800 60fps 영상이라 DERP 강제 = throughput cap = 사망. **UAC 1회를 없애려고 P2P 품질을 후퇴시키는 거래**라 방향이 반대다.
  2. **미감사 암호 + 저자 본인의 프로덕션 금지 경고**. *"assume there could be a critical security hole meaning anything you send or receive could be in the clear on the public Internet"*, 그리고 링크하는 모든 코드에 `TS_RS_EXPERIMENT=this_is_unstable_software` 강제. 우리 위협 모델은 hub 가 관리자 자격증명을 나르고 agent 가 **임의 PowerShell 을 실행**하며 스트림이 **학생 노트북 화면 전체**다. 평문 노출 가능성만으로 즉시 탈락.
  3. **핵심 제약은 그대로**: 여전히 유저스페이스 netstack 이라 `100.x` 가 프로세스 안에만 있다. streamhost 6소켓(+moonclient)을 전부 netstack API 로 재배선해야 하는 분량은 줄지 않는다. 줄어든 건 "Go FFI" 항목 하나뿐.
  4. **툴체인**: MSRV 1.94.1 + edition 2024. 현재 우리 `rustc 1.90.0` 에서는 컴파일 자체가 안 된다.
  5. **성숙도**: star 2 / fork 1, pre-1.0, 호환성 보장 없음, rolling MSRV.
  - **재검토 트리거(셋 다 충족 시)**: (a) 서드파티 감사 완료 + `TS_RS_EXPERIMENT` 게이트 제거, (b) symmetric-NAT 홀펀칭 구현, (c) 1.0 릴리스. 그때는 드라이버 0·방화벽 규칙 0·UAC 0 의 단일 Rust 바이너리라 진지하게 값어치가 있다.
