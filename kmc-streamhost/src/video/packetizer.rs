...
//!
//! Reed-Solomon FEC(호출자가 지정한 parity%) + multi-FEC 블록으로 패킷 손실을 복구한다
//! (무선/인터넷 경유 대비).
//!
//! 와이어 레이아웃(RTP 헤더 / NvVideoPacket / VideoFrameHeader)은 `kmc-gsproto` 가 **단일 소스**로
//! 정의하고, 클라이언트(`kmc-moonclient`)의 디패킷타이저가 같은 정의를 읽는다. 이 모듈의 책임은
//! 레이아웃이 아니라 **분할 정책**이다: shard 개수 산정, FEC 블록 분할, parity 생성, RS 이후 헤더 패치.

use kmc_gsproto::{
    parity_shard_count, rtp_flag, NvVideoPacket, RtpHeader, VideoFrameHeader, MAX_FEC_BLOCKS,
    MAX_SHARDS, NV_PACKET_OFFSET, NV_VIDEO_PACKET_SIZE, PAYLOAD_OFFSET, RTP_VERSION_FLAGS,
...
pub fn packetize_frame(
    ...
    for block_index in 0..nr_blocks {
        ...
        // parity 개수는 반드시 gsproto 의 공유 함수로 뽑고, 와이어에는 **요청한** fec_percentage 를
        // 싣는다. 수신 측은 그 퍼센트로 같은 함수를 돌려 개수를 복원한다 — RS 생성행렬의 parity 행이
        // 총 parity 개수에 의존하기 때문에, 개수가 하나라도 어긋나면 복구 결과가 조용히 깨진다.
        let nr_parity = parity_shard_count(block_data_shards, fec_percentage);
        ...
        for i in 0..block_data_shards {
            ...
            RtpHeader { sequence_number: cur_seq as u16, timestamp: rtp_timestamp, ssrc: 0 }
                .write(shard);
        ...
        // parity shard 헤더 패치.
        // RS parity 는 shard **전체(헤더 포함)** 위에서 계산되므로, 아래에서 덮어쓰지 않는 바이트
        // (timestamp/ssrc/spi/flags)는 parity 값을 그대로 유지해야 한다. 수신 측은 flags 가 아니라
        // fec_info 의 shard_index 로 parity 를 식별한다.
        for i in 0..nr_parity {
            ...
            let fec_info =
                NvVideoPacket::pack_fec_info(block_shard_index, block_data_shards, fec_percentage);
