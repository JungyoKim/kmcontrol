...
const MSG_FRAME_FEC_STATUS: u16 = kmc_gsproto::MSG_FRAME_FEC_STATUS; // 손실 프레임 FEC 상태.
...
fn handle_packet(buf: &[u8], session: &SessionState, trigger: &VideoTrigger, idr_req: &AtomicBool, bitrate: &crate::bitrate::BitrateController) {
    ...
    match effective_type {
        ...
        MSG_FRAME_FEC_STATUS => {
            // FEC 로 복구된 프레임이어도 손실은 보고된다 — 네트워크 열화의 조기 지표.
            let loss = kmc_gsproto::FrameFecStatus::parse(&payload)
                .and_then(|s| s.loss_fraction());
            if let Some(frac) = loss {
